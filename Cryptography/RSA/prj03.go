...
func aliceEncrypt(bobEncPub *rsa.PublicKey, plaintxt []byte) []byte {
	...
	ciphertxt, err := rsa.EncryptOAEP(sha256.New(), rand.Reader, bobEncPub, []byte(plaintxt), []byte(label))
	if err == nil {
		return ciphertxt
	}
	return nil
...
func bobDecrypt(bobEnc *rsa.PrivateKey, ciphertxt []byte) []byte {
	...
	plaintxt_decode, err := rsa.DecryptOAEP(sha256.New(), rand.Reader, bobEnc, ciphertxt, []byte(label))
	if err == nil {
		plaintxt := string(plaintxt_decode)
